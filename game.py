...
import pygame
import sys
from leap_motion.leap_project_listener import start_leap_listener
...
def main():
    global level
    display_menu(screen)  # Display the menu before starting the game

    running = True
    worldstate = WorldState()
    start_leap_listener(worldstate, SCREEN_WIDTH, handle_shooting)  # Pass handle_shooting as an argument

    total_enemies = len(worldstate.enemies)  # Store the initial number of enemies
    boss = None  # Boss initialization
    ...
    while running:
        ...
        # Boss level condition
        boss_level = (level % 3) == 0  # Boss appears every 3 levels
        ...
        if boss_level and boss is None:
            ...
            boss.maxHP = 30 + (level // 3) * 10
            boss.health = boss.maxHP
        ...
        if boss:
            ...
            remaining_enemies = len(worldstate.enemies)

            if not remaining_enemies:
                display_message(screen, f"Level {level} Complete!\nNext Level!", duration=1, height=400)
                level += 1
                worldstate.enemies = [Enemy(x * 60 + 50, y * 60 + 50) for x in range(8) for y in range(3)]
                total_enemies = len(worldstate.enemies)  # Reset enemy count for new level
                
                # Adjust speed based on the level
                for enemy in worldstate.enemies:
                    enemy.speed += (level / 2) if level > 4 else level
        ...
        # Draw the enemy progress bar at the bottom
        draw_enemy_bar(screen, total_enemies, remaining_enemies)
